    ...
    ***********************************-->
                    ...
                    <div class="collapse navbar-collapse justify-content-between">
                        <div class="header-left">
							<div class="input-group search-area right d-lg-inline-flex d-none">
								<input type="text" id="searchda" class="form-control" placeholder="Recherche...">
            ...
            <div class="deznav-scroll">
				...
				<ul class="metismenu" id="menu">
					...
					<li><a href="sta_depa.php" class="ai-icon" aria-expanded="false">
							<i class="flaticon-381-settings-2"></i>
							<span class="nav-text">Stagiaire et Module</span>
						</a>
					</li>
                </ul>
			</div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-responsive-md">
                                        <thead>
                                            <tr>
                                                <th><strong>Stagiaire </strong></th>
                                                <th><strong>Module</strong></th>
                                                <th><strong>Montant Paye</strong></th>
                                                <th><strong>Montant Restant</strong></th>
                                                <th><strong>Somme a payer</strong></th>
												<th>Action</th>
                                            </tr>
                                        </thead>
										<?php
										$sql="SELECT 
                                            ...
                                            motant_paye
										FROM
                                        tbl_stagiaire_module AS stam INNER JOIN tbl_stagiares AS sta ON(stam.id_sta=sta.id_sta) INNER JOIN tbl_module AS modu ON(stam.id_mod=modu.id_mod)";
											$recupdata=$my_bd->prepare($sql);
											$recupdata->execute();
											$sta_modgiaire_mdule=$recupdata->fetchALL(PDO::FETCH_ASSOC);
										?>
                                        <tbody id="searchosx">
											<?php
											foreach($sta_modgiaire_mdule as $sta_mod){
											?>
                                            <tr>
                                                <td><?=$sta_mod['nom_sta']?></td>
												<td><?=$sta_mod['titre_mod']?></td>
                                                <td><?=$sta_mod['motant_paye']?> Fbu</td>
                                                <td class="colorreste"><?=$prixtotal - $sta_mod['motant_paye']?> Fbu</td>
                                                <td><?=$prixtotal?> Fbu</td>
                                                <td>
													<div class="d-flex">
														<button type="submit" class="buton_add" data-bs-toggle="modal" data-bs-target="#modi<?=$sta_mod['id_stamod']?>">&nbsp;&nbsp;<i class="fa fa-pencil icon_bo"></i>&nbsp;&nbsp;</button>&nbsp;&nbsp;
														<button type="submit" class="buton_close" data-bs-toggle="modal" data-bs-target="#supr<?=$sta_mod['id_stamod']?>">&nbsp;&nbsp;<i class="fa fa-trash icon_bo"></i>&nbsp;&nbsp;</button>
													</div>
												</td>
                                            </tr>
                                        </tbody>
																				        				<!-- Modal Update-->
										<div class="modal fade" id="modi<?=$sta_mod['id_stamod']?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
											<div class="modal-dialog">
												<div class="modal-content">
													<div class="modal-header">
														<h5 class="modal-title" id="exampleModalLabel"><strong>Ajouter Profile</strong></h5>
														<button type="button" class="btn-close btn_modalclose" data-bs-dismiss="modal" aria-label="Close">close</button>
													</div>
													<form method="POST">
                                                    <div class="modal-body">
                                                        <div class="row">
                                                            <input type="hidden" name="input_id"  value="<?=$sta_mod['id_stamod']?>">
                                                            <div class="col-12 col-lg-6  py-2">
                                                                ...
                                                                <div class="col-12 col-lg-6  py-2">
                                                                        <select name="input_sta" class="form-control">
                                                                            <option>Choisie Cours</option>
                                                                            <?php
                                                                                $recudata=$my_bd->query("SELECT * FROM tbl_stagiares ORDER BY id_sta DESC");
                                                                                if($recudata->rowCount()>0){
                                                                                    while($checkdata=$recudata->fetch()){
                                                                                    $id_cours=$checkdata['id_sta'];
                                                                                    $nom_cours=$checkdata['nom_sta'];
                                                                                ?>
                                                                                <option value="<?=$id_cours?>"><?=$nom_cours?></option>
                                                                                <?php
                                                                                    }
                                                                ...
                                                                <div class="col-12 col-lg-12  py-2">
                                                                    <input type="text" class="form-control" value="<?=$sta_mod['matric_sta']?>" name="input_matri" placeholder="Matricule">
                                                                </div>
                                                            </div>
                                                        </div>
											...
											<!-- Modal suprimer-->
											<div class="modal fade" id="supr<?=$sta_mod['id_stamod']?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
												<div class="modal-dialog">
													<div class="modal-content">
													<div class="modal-header">
														<h5 class="modal-title" id="exampleModalLabel"><strong>Supprimer Affectation</strong></h5>
														<button type="button" class="btn-close btn_modalclose" data-bs-dismiss="modal" aria-label="Close">close</button>
													</div>
													<form method="POST">
														<div class="modal-body">
															<div class="row">
                                                            <input type="hidden" class="form-control" name="input_id" value="<?=$sta_mod['id_stamod']?>">
															<p class="deleteclass">Veux-tu supprimer Cette affectation de <?=$sta_mod['nom_sta']?> au module <?=$sta_mod['titre_mod']?> &nbsp;<span class="utinom"><?=$sta_mod['matric_sta']?></span></p>
															</div>
														</div>
														<div class="modal-footer">
	...
	<div class="modal fade" id="addsecretaire" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel"><strong>Affecter Stagiaire et Module</strong></h5>
					<button type="button" class="btn-close btn_modalclose" data-bs-dismiss="modal" aria-label="Close">close</button>
				</div>
				<form method="POST">
					<div class="modal-body">
						...
						<div class="col-12 col-lg-6  py-2">
                            <select name="input_sta" class="form-control">
                                <option>Choisie Stagiaire <?=$prixtotal?></option>
                                <?php
                                    $recudata=$my_bd->query("SELECT * FROM tbl_stagiares ORDER BY id_sta DESC");
                                    if($recudata->rowCount()>0){
                                        while($checkdata=$recudata->fetch()){
                                        $id_sta=$checkdata['id_sta'];
                                        $nom_sta=$checkdata['nom_sta'];
                                    ?>
                                    <option value="<?=$id_sta?>"><?=$nom_sta?></option>
                                    <?php
                                        }
                                        }
                                ?>
							...
							<div class="col-12 col-lg-6  py-2">
                                    <select name="input_module" class="form-control">
                                        <option>Choisie Module</option>
                                        <?php
                                            $recudata=$my_bd->query("SELECT * FROM tbl_module ORDER BY id_mod DESC");
                                            if($recudata->rowCount()>0){
                                                while($checkdata=$recudata->fetch()){
                                                $id_mod=$checkdata['id_mod'];
                                                $titre_mod=$checkdata['titre_mod'];
                                            ?>
                                            <option value="<?=$id_mod?>"><?=$titre_mod?></option>
                                            <?php
                                                }
                                                }
                                        ?>
							...
							<div class="col-12 col-lg-12  py-2">
								...
								<input type="number" class="form-control" name="input_montpaye">
							</div>
                            <!-- <div class="col-12 col-lg-6  py-2">
                                <label for="">Montant R</label>
								<input type="text" class="form-control" name="input_matri" placeholder="Matricule">
							</div> -->
						</div>
					</div>
					<div class="modal-footer">
