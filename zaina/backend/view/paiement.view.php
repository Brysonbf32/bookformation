...
<body>
    ...
    <div id="main-wrapper">
		<div class="nav-header">
										...
										<table class="table table-responsive-md">
											<thead>
												<tr>
													...
													<th><strong>Departemnt</strong></th>
													<th><strong>Module</strong></th>
													<th><strong>Duree</strong></th>
													<th><strong>Montant Payee</strong></th>
													<th><strong>Montant Restant</strong></th>
													<th><strong>Totalite</strong></th>
												</tr>
											</thead>
											<?php
											$sql="SELECT 
											...
											motant_paye
										FROM
										tbl_stagiaire_module AS stam INNER JOIN tbl_stagiares AS sta ON(stam.id_sta=sta.id_sta) INNER JOIN tbl_module AS modu ON(stam.id_mod=modu.id_mod)INNER JOIN tbl_departement AS dep ON (sta.id_depart= dep.id_depart)";
											$recupdata=$my_bd->prepare($sql);
											$recupdata->execute();
											$paimentmodule=$recupdata->fetchALL(PDO::FETCH_ASSOC);
											?>
											<tbody>
												<?php
												foreach($paimentmodule as $paimod){
												?>
												<tr>
													<td><?=$paimod['nom_sta']?></td>
													<td><?=$paimod['nom_depart']?></td>
													<td><?=$paimod['titre_mod']?></td>
													<td><?=$paimod['duree_mod']?> Mois</td>
													<td><?=$paimod['motant_paye']?> Fbu</td>
													<td class="colorreste"><?=$prixtotal - $paimod['motant_paye']?> Fbu</td>
													<td><?=$prixtotal?> Fbu</td>
												</tr>
											</tbody>
													<!-- Modal suprimer-->
												<div class="modal fade" id="supr<?=$paimod['id_stamod']?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
													<div class="modal-dialog">
														...
														<form method="POST">
															<div class="modal-body">
																<div class="row">
																<input type="hidden" class="form-control" name="input_idmo" value="<?=$paimod['id_stamod']?>">
																<p class="deleteclass">Veux-tu supprimer le module <span class="utinom"><?=$paimod['ref_detail']?></span></p>
																</div>
															</div>
			...
			<div class="footer">
				<div class="copyright">
					<p>Copyright © Designed &amp; Developed by <a href="../index.htm" target="_blank">Jabf32</a> 2023</p>
				</div>
			</div>
		</div>
